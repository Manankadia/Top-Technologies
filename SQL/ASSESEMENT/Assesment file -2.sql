...
select month(sale_amount) , sum(sale_amount) from sales group by month(sale_amount);
...
select category from (select category , month(sale_date),sum(sale_amount) from sales group by category,month(sale_date) order by category,month) group by category having min(total_sales) < max(total_sales);
...
select count(distinct month(sale_date)) from sales;
