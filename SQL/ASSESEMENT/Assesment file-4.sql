...
select brand,model from cars where mileage = 22000;
select brand,model,mileage from cars where mileage = (select max(mileage) from cars);

-- 3. List cars with prices higher than the average price of cars with the fueltype = 'Gasoline'.
 select avg(price) from cars;
 ...
 select  avg(price) from cars;
  select  avg(price) from cars where year = 2021;
  ...
  select brand from (select brand,count(*) as model_count from cars group by brand) as brand_models where model_count >2;
  
  -- 9. List cars with prices higher than the average price of cars with the same transmission type and fuel type.
select avg(price) from cars where transmission_type = 
