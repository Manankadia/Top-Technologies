...
select P.productname , O.quantity from orders as P
join customers as O
...
select orders .* from orders join customers on orders.customerID =customers.customerID where customers.state = 'california';

-- 14. Show the customers along with the number of orders they've placed.
select customers.customerID , customers.firstname , count(orders.orderID) as num_of orders 
from customers 
