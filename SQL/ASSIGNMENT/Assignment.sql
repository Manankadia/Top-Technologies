 ...
 select hire_date , concat(first_name ," ",last_name) as full_name from employees;
  ...
  -- 3. Display the ename concatenated with the job ID, separated by a comma and space, and name the column Employee and Title
   select concat(first_name, last_name , ', ', job_id) as 'Employee and Title' from employees;
    ...
    select hire_date , name as concat(first_name," ",last_name) , department_id from employees
    where job_id = "cleark";
     ...
     where salary >2000;
      ...
      select concat(first_name ," " ,last_name) as full_name ,hire_date as "start date" from employees;
      ...
      select last_name as name , job_id as job_title
      from employees
      ...
      select concat(first_name , ' ', last_name) as last_name, job_id as job, salary as salary
      from employees 
      where job_id in ('SA_REP', 'ST_CLERK')
      and sal not in (2500, 3500, 5000);
      
      -- 13) Display the maximum, minimum and average salary and commission earned.
      select max(salary) as max_salary , min(salary) as min_salary , avg(salary) as avg_salary , max(commission_pct) as max_commission_pct , min(commission_pct) as min_commission_pct , avg(commission_pct) as avg_commission_pct from employees;
      
       -- 14) Display the department number, total salary payout and total commission payout for each department. 
       select department_id , sum(salary) as total_salary_payout , sum(commision_pct) as total_commission_pct_payout from employees group by department_id ;
       
       -- 15) Display the department number and number of employees in each department. 
       select departments.department_id , departments.department_name ,count(employee.employee_id) as num_employees
       from departments
       join employees 
       on departments.department_id = employees.department_id;
       
       -- 16) Display the department number and total salary of employees in each department.
       select departments.department_id , departments.department_name , sum(employees.salary) as total_salary
       from departments 
       ...
       select concat(first_name," ",last_name) as full_name from employees where commission_pct = 0 order by 1;
       
       -- 18) 6) Display the employees name, department id and commission. If an Employee doesn't earn the commission, then display as 'No commission'. Name the columns appropriately
       select concat(first_name, ' ', last_name) as employee_name, department_id,
         coalesce(commission, 'No commission') as commission
         from employees;
         
         -- 19) Display the employee's name, department id who have the first name same as another employee in the same department
         select e1.concat(first_name, ' ', last_name), e1.department_id
         from employees e1
         join employees e2 on e1.concat(first_name, ' ', last_name) = e2.concat(first_name, ' ', last_name)
        ...
        select  count(e.employee_id) as employee_count ,m.manager_id
        from managers m
        ...
        select m.manager_name , e.employee_name , e.department_id , e.salary
        from employee e
        join mangers m on e.manager_id=m.manager_id
        where substring(e.employee_name ,2 ,1) = "a"
        group by m.manager_name , e.employee_name , e.department_id , e.salary;
        ...
        -- 25) Write a query to display the last name, department number, department name for all employees.
        select e.last_name , e.department_id , d.department_name
        from employees e
        join departments d on e.department_id = d.department_id;
        
        -- 26) Create a unique list of all jobs that are in department 4. Include the location of the department in the output.
        ...
        -- 27) Write a query to display the employee last name,department name,location id and city of all employees who earn commission.
        ...
        -- 28) Display the employee last name and department name of all employees who have an 'a' in their last name 
        select 
        e.last_name,
        d.department_name
        from 
        ...
        -- 29) Write a query to display the last name,job,department number and department name for all employees who work in ATLANTA. 
        ...
        -- 30) Display the employee last name and employee number along with their manager's last name and manager number.
        select 
        ...
        -- 31) Display the employee last name and employee number along with their manager's last name and manager number (including the employees who have no manager).
        select
        e1.last_name as employee_last_name,
        e1.employee_id as employee_number,
        coalesce(e2.last_name, 'No Manager') as manager_last_name,
        coalesce(e2.employee_id, -1) as manager_number
        from 
        employees e1
        left join
        employees e2 on e1.manager_id = e2.employee_id;
        
        -- 32) Create a query that displays employees last name,department number,and all the employees who work in the same department as a given employee. 
        select 
        e1.last_name as employee_last_name,
